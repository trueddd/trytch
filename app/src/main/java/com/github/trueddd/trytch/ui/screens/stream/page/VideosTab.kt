...
data object Videos : PageTab(R.string.streamer_screen_videos_tab) {
...
fun VideosContent(
    user: User = User.test(),
    videosState: StreamerPageState.VideosState = StreamerPageState.VideosState.test(),
    loadCallback: (Videos.LoadOptions) -> Unit = {},
) {
    LaunchedEffect(Unit) {
        loadCallback(Videos.LoadOptions(user))
    }
    Box(
        modifier = Modifier
            .fillMaxWidth()
                    ...
                    Row(
                        ...
                        modifier = Modifier
                            ...
                            .clickable(
                                ...
                                onClick = { loadCallback(Videos.LoadOptions(user)) },
                            )
                    ) {
                        Box(
                            modifier = Modifier
                            ...
                            CoilImage(
                                model = buildImageRequest(item.getThumbnailUrl(width = 320, height = 180)),
